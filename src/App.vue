...
<template>
    ...
    <div class="container max-h-max font-sans w-[21rem]  rounded-xl  bg-sky-50 flex flex-col justify-start shadow-xl py-10 px-12">
      ...
      <form class="flex flex-col gap-4 max-w-max" @submit.prevent="addItemToArray()">
        ...
        <button
          type="submit"
          class="text-sm bg-indigo-200 h-10 rounded-lg px-6  fs focus:outline-none focus:ring focus:ring-violet-600 shadow-md active:shadow-sm active:translate-y-[0.1rem]"
      ...
      <div class="container max-h-max mx-auto font-sans max-w-max md:mt-0 mt-8  rounded-xl bg-sky-50 flex flex-col justify-start shadow-xl py-10 px-12">
        ...
        <span :class="{buyDone: item.isPurchase, highPriority: item.priority.isPriority }">{{item.quantity}} {{ item.item }}</span> |
         <button @click.prevent="deleteItem(item.id)" class="text-sm font-bold bg-red-400 text-w px-2 py-1 rounded-lg" type="button">מחק</button>
          <button @click.prevent="togglePurchase(item)" class="text-sm bg-lime-400 text-w px-2 font-bold py-1 rounded-lg" type="button">נקנה</button>
        </li>
      </ul>
      </div>
  </div>
